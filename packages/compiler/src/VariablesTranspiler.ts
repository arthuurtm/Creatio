...
export class VariablesTranspiler {
  ...
  public transpile(): string {
    ...
    // 1. Fase de Registro: Encontra as declarações (Variáveis e Funções) e registra seus IDs na Tabela de Símbolos
    ...
    for (const funcNode of functionDeclarations) {
      const { name } = funcNode.data.ast.params;
      const safeName = this.sanitizeVariableName(name);
      this.symbolTable.set(funcNode.id, safeName);
    }

    // 2. Processa as declarações primeiro (estáticas no topo do código)
    for (const decNode of variableDeclarations) {
      const rawESTree = decNode.data?.ast?.estree;
      if (rawESTree) {
  ...
  private resolveIdentifiers(node: any): void {
    ...
    if (node.type === "Identifier" && typeof node.name === "string") {
      ...
      // 1. Se for ID de uma variável declarada, substitui pelo nome real
      if (this.symbolTable.has(name)) {
        node.name = this.symbolTable.get(name)!;
      } 
      // 2. Se for uma expressão que contém IDs de variáveis, substitui cada ID ocorrido
      else {
      ...
      // 3. Se o nome resolvido for número literal disfarçado de Identifier, converte para Literal
